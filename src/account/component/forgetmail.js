...
class Forgetmail extends Component {
    state = {
        display:true
    }
    submit = () => {
this.setState({display:false})
        ...
        fetch('/sendpass', options)
            .then((res) => res.json())
            .then((data) => {
                if (data.exist === true) {
                    alert('chck your email, and enter code')
                    this.props.history.push('/forgetauth/' + user_email + '/' + digit)
                }
                else if (data.exist === false) {
                    alert("You are't the part of our society" )
...
export default withRouter(Forgetmail)